...
c     &(q2/(2.d0*mp*x)),(q2/(2.d0*mp*nu)),(z*nu)
...
c Check that <cos(phi)> and <cos(2phi)> are < 1
      ...
      m_cos_phi=sqrt(pt2/q2)*(2.d0-y)*rtz*H3m/(2.d0*cterm)
...
c      if(Ebeam.eq.27.d0) print*,'<cos(phi)> ',m_cos_phi
      ...
      H3m=0.9d0*sign(1.d0,m_cos_phi)*(2.d0*cterm)/(sqrt(pt2/q2)*(2.d0-y)*rtz)/4.d0
...
c      if(Ebeam.eq.27.d0) print*,'<cos(phi)> ',m_cos_2phi
      ...
      H4m=0.9d0*sign(1.d0,m_cos_2phi)*(2.d0*cterm)/(pt2/q2*rtz**2)/4.d0
c      if(Ebeam.eq.27.d0) print*,'Correct H4= ',H4m
      endif
c      if(Ebeam.eq.27.d0) print*,'final ',(x*y**2*H1+rt*H2),
...
c      H2z=2.d0*z/(nu*mp**2)*(H2+aa/x*H3m+
